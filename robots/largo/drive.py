...
address = 0x23
myGPIO = 12
pi.set_mode(myGPIO, pigpio.OUTPUT)
...
def adjust_track(track1, track2, dir):
    if dir == 1:
        pi.write(track1, True)
        pi.write(track2, False)
    if dir == -1:
        pi.write(track1, False)
        pi.write(track2, True)
    if dir == 0:
        pi.write(track1, False)
        pi.write(track2, False)
...
while True:
    ...
    framecol.update_one(
        {"_id": "umberto"},
        {"$set": {"lux": lux}, "$inc": {"energy": int(lux / luxspeed + 10)}},
        upsert=True,
    )
    energy = framecol.find_one_and_update(
        {"_id": "umberto"},
        {"$set": {"lux": lux}, "$min": {"energy": 10000}},
        projection={"energy": 1},
    ...
    if dir == None:
        count = count + 1
    if dir == "open":
        pi.set_servo_pulsewidth(myGPIO, 1000)
    if dir == "close":
        pi.set_servo_pulsewidth(myGPIO, 2000)
    if dir == "fwd":
        left = 1
        right = 1
        runfor = 1
